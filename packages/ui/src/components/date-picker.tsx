...
import { type Locale, enUS, th } from "date-fns/locale"
import {
...
function genYears(yearRange = 50, buddhist = false) {
  const today = new Date()
  return Array.from({ length: yearRange * 2 + 1 }, (_, i) => {
    const ceYear = today.getFullYear() - yearRange + i
    return {
      value: ceYear,
      label: buddhist ? (ceYear + 543).toString() : ceYear.toString(),
    }
  })
}

/** แปลงปีในข้อความ format ที่ได้จาก date-fns ให้เป็น พ.ศ. */
function toBuddhistYear(formatted: string, date: Date): string {
  const ceYear = date.getFullYear()
  const beYear = ceYear + 543
  // แทนเฉพาะปี 4 หลัก (ค.ศ. → พ.ศ.) ครั้งเดียว
...
function Calendar({
  ...
  buddhist = false,
  ...props
}: DayPickerProps & { yearRange?: number; buddhist?: boolean }) {
  ...
  const YEARS = React.useMemo(() => genYears(yearRange, buddhist), [])
            ...
            <div className="inline-flex gap-2">
              ...
              >
                <SelectTrigger className="w-fit gap-1 border-none p-0 focus:bg-accent focus:text-accent-foreground">
                  <SelectValue>
                    {buddhist
                      ? (calendarMonth.date.getFullYear() + 543).toString()
...
type DateTimePickerProps = {
  ...
  /**
   * แสดงปีแบบพุทธศักราช (พ.ศ.) โดยบวก 543 จากปี ค.ศ.
   * ค่าที่เก็บใน Date object ยังเป็น ค.ศ. ตามเดิม
   **/
  buddhist?: boolean
} & Pick<
  DayPickerProps,
